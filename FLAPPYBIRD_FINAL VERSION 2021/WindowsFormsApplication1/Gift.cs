        ...
        public void GetGift(Form form, Pipe pipe)
        {
            ...
            if (pipe.X_pipePairs1 + pipe.recPipe.Width <= 0)
            {
                ...
                if (gift_Count == randomNext_Gift)
                {
                    ...
                    DrawGift(form, pipe);
                    gift_Count = 0;
                    randomNext_Gift = random.Next(12, 13);
                }
            }
        }

        public void DrawGift(Form form, Pipe pipe)
        {
            X_Gift = form.Width + 150 + pipe.recPipe.Width + random.Next(0, distance_2pipe);

            Y_Gift = random.Next(0, form.Height - 90);
